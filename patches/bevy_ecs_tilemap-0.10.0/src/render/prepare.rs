...
pub(crate) fn prepare(
    ...
    for tile in extracted_tiles.iter() {
        ...
        // Gracefully handle case where tilemap has been despawned
        let Ok((
            _entity,
            transform,
            tile_size,
            ...
            visibility,
            frustum_culling,
        )) = extracted_tilemaps.get(tile.tilemap_id.0)
        else {
            // Tilemap entity no longer exists (despawned), skip this tile
            continue;
        };
