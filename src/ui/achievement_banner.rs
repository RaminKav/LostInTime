    ...
    for event in events.read() {
        ...
        let banner_entity = commands
            .spawn((
                (
                    Sprite {
                        color: Color::srgba(0.08, 0.08, 0.08, 0.93),
        ...
        commands
            .spawn(
                gf::DISPLAY
                    .text(&asset_server, "Achievement!", Color::WHITE)
            ...
            .insert(ChildOf(banner_entity));

        commands
            .spawn(
                gf::BODY
                    .text(&asset_server, title_text, Color::WHITE)
                    .anchor(Anchor::TOP_LEFT)
                    .with_transform(Transform {
...
pub fn debug_trigger_achievement_banner(
    ...
    mut events: MessageWriter<AchievementUnlockedEvent>,
) {
    if *DEBUG && key_input.just_pressed(KeyCode::KeyQ) {
